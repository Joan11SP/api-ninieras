...
module.exports =
{
    ...
    SQL_EXISTE_LOGIN_CLIENTE:"select count(*) existe from "+config.NAME_DB+".login_cliente where id_login_cliente = ?;",
    SQL_EXISTE_LOGIN_NINERA:"select count(*) existe from "+config.NAME_DB+".login_niniera where id_login_niniera = ?;",
    SQL_EXISTE_PER_CLIENTE:"select count(*) existe from "+config.NAME_DB+".login_cliente where id_persona = ?;",
    SQL_EXISTE_PER_NINIERA:"select count(*) existe from "+config.NAME_DB+".login_niniera where id_persona = ?;",
    ...
    SQL_VAL_EXISTE_SOL_SERVICIO:"select count(*) existe from  "+config.NAME_DB+".solicitud_servicio where id_cliente = ? and estado in(1,2);",
    SQL_ADD_SOL_SERVICIO: "insert into "+config.NAME_DB+".solicitud_servicio (id_cliente, id_ubicacion, hora_entrada, hora_salida, dia_inicia, dia_fin, numero_dias, codigo_dactilar, identificacion,permite_uso_wifi, descripcion,total_horas, estado) values (?,?,?,?,?,?,?,?,?,?,?,?,1);",
    SQL_ADD_PERS_ANIMAL_CUIDADO:"insert into "+config.NAME_DB+".persona_animal_cuidado (id_cliente,nombres,edad,sexo,capacidad_diferente,info_adicional,es_persona_o_animal) values (?,?,?,?,?,?,?);",
    ...
    SQL_VAL_EXISTE_CUIDAD: "select count(*) existe from "+config.NAME_DB+".persona_animal_cuidado where id_cuidados = ? and id_cliente = ?;",
    SQL_GET_ONE_SERVICIOS: "select s.id_ubicacion,nombres, ifnull(foto,'SF') foto, hora_entrada, hora_salida, numero_dias, total_horas,date_format(dia_inicia,'%Y-%m-%d') dia_inicia, date_format(dia_fin,'%Y-%m-%d') dia_fin, cast(permite_uso_wifi=1 as signed integer) permite_uso_wifi, descripcion"
                           + " from "+config.NAME_DB+".persona p, "+config.NAME_DB+".solicitud_servicio s, db_ninieras.login_cliente lc"
                           + " where s.id_cliente = lc.id_login_cliente and"
                           + " p.id_persona = lc.id_persona and s.id_solicitud_servicio = ? and s.id_cliente = ? and s.estado = ?;",
    SQL_GET_UBI_ONE_SERVICIO: "select latitud, longitud, referencia, calles from "+config.NAME_DB+".ubicacion u, "+config.NAME_DB+".solicitud_servicio s"
                                +" where s.id_ubicacion = u.id_ubicacion and s.id_cliente = u.id_cliente"
                                +" and s.id_solicitud_servicio = ? and s.id_cliente = ? and s.id_ubicacion = ? and s.estado = ?;",
    SQL_ADD_UBICACION: "insert into "+config.NAME_DB+".ubicacion (ubicacion_nombre,latitud,longitud,referencia,calles,id_cliente) values (?, ?, ?, ?, ?, ?);",
    SQL_GET_CUIDADO_SERVICIO: "select d.id_cuidados, nombres, edad, sexo, cast(capacidad_diferente=1 as signed integer) capacidad_diferente,"
                                ...
                                +" from "+config.NAME_DB+".detalle_cuidado_servicio d, "+config.NAME_DB+".persona_animal_cuidado p"
                                +" where d.id_cuidados = p.id_cuidados and d.id_solicitud_servicio = ?;",
    SQL_ADD_PROPUESTA_SERVICIO: "insert into "+config.NAME_DB+".propuesta_servicio (id_solicitud_servicio,id_niniera) values (?,?);",
    SQL_GET_INFO_NOTIFI_PROPUESTA: "select ifnull(token_firebase, 'STF') info  from "+config.NAME_DB+".solicitud_servicio s, "+config.NAME_DB+".login_cliente lc"
                                   +" where s.id_cliente = lc.id_login_cliente and s.estado = 1 and s.id_solicitud_servicio = ? union all"
                                   +" select nombres from "+config.NAME_DB+".persona p, "+config.NAME_DB+".login_niniera ln where p.id_persona = ln.id_persona"
                                   +" and ln.estado_login = 1 and  ln.id_login_niniera = ?;",
    SQL_VAL_SOLICITUD_VIGENTE: "select count(*) existe from "+config.NAME_DB+".solicitud_servicio where id_solicitud_servicio = 1 and estado = 1;",
    SQL_VAL_EXISTE_PROPUESTA: "select count(*) existe from db_ninieras.propuesta_servicio where id_solicitud_servicio = ? and id_niniera = ?;"
